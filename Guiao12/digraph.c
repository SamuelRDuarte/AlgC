//Samuel Rodrigues Duarte 89222
//Renato Lima Valente 89077
...
int VertexType (PtDigraph pdig, unsigned int pv)
{
  ...
  if(Node == NULL) return NO_VERTEX;

  PtVertex Vertice = (PtVertex)(Node->PtElem);

  if(Vertice->InDeg == 0){
	  if(Vertice->OutDeg == 0) return DISC;
	  return SOURCE;
  }
...
int VertexClassification (PtDigraph pdig, unsigned int pv, double *pclass)
{
  ...
  *pclass = ((double)Vertice->InDeg + (double)Vertice->OutDeg)/ ((double)(2*(pdig->NVertexes-1)));
  if(pclass == NULL) return NULL_PTR;
...
PtDigraph DigraphTranspose (PtDigraph pdig)
{
  /* Insira o seu c�digo - Insert your code */
  if(pdig == NULL) return NULL;
  if(pdig->NVertexes == 0) return NULL;

  PtDigraph digTrans = Create(pdig->Type);
  if(digTrans == NULL) return NULL;

  PtBiNode Vert1; 
  for (Vert1 = pdig->Head; Vert1 != NULL; Vert1 = Vert1->PtNext)
  {
	  if(InVertex(digTrans,Vert1->Number)){ Destroy(&digTrans); return NULL;}
  }

  for (Vert1 = pdig->Head; Vert1 != NULL; Vert1 = Vert1->PtNext)
  {
	  PtBiNode Aresta;
	  for(Aresta = Vert1->PtAdj;Aresta!=NULL;Aresta = Aresta->PtNext){
		  PtEdge Edge =(PtEdge) Aresta->PtElem;
		  if(InEdge(digTrans,Aresta->Number,Vert1->Number,Edge->Cost)){
			  Destroy(&digTrans);
			  return NULL;
		  }
	  }
...
int DigraphEdgeSplit (PtDigraph pdig, unsigned int pve, unsigned int pvi)
{
  ...
  PtBiNode VerticeOut = OutPosition(pdig->Head,pve);

  int exits = 0;
  int custo = 0;
  PtBiNode Aresta;
  PtEdge Edge;
  for(Aresta=VerticeOut->PtAdj;Aresta!=NULL;Aresta=Aresta->PtNext){
	  if(Aresta->Number == pvi){
		exits = 1; 
		Edge = Aresta->PtElem;
		custo = Edge->Cost;
	  } 
  }
  if(exits == 0) return NO_EDGE;

  int num = 0;
  PtBiNode Vert1;
  for(Vert1 = pdig->Head;Vert1->PtNext!=NULL;Vert1= Vert1->PtNext);
  num = Vert1->Number;

  if(InVertex(pdig,num+1) == NO_MEM) return NO_MEM;
  if(InEdge(pdig,pve,num+1,custo/2)){return NO_MEM;}
  if(InEdge(pdig,num+1,pvi,(custo+1)/2)){return NO_MEM;}
...
//novos vertices
  if(InVertex(pdig,num+1) == NO_MEM) return NO_MEM;
  if(InVertex(pdig,num+2) == NO_MEM) return NO_MEM;
  if(InEdge(pdig,num+1,num+2,0)){return NO_MEM;}
  
  //PtBiNode LastVert = OutPosition(pdig->Head,num+2);
  PtBiNode Aresta;
  for(Aresta = Vert->PtAdj;Aresta!=NULL; Aresta = Aresta->PtNext){
	  PtEdge Edge = (PtEdge) Aresta->PtElem;
	  if(InEdge(pdig,num+2,Aresta->Number,Edge->Cost)) return NO_MEM; 
  }
  ...
  for(Vert1 = pdig->Head;Vert1!=NULL;Vert1= Vert1->PtNext){
	  for(Aresta = Vert1->PtAdj;Aresta!=NULL; Aresta = Aresta->PtNext){
		  if(Aresta->Number == pv){
			  PtEdge Edge = (PtEdge) Aresta->PtElem;
			  if(InEdge(pdig,Vert1->Number,num+1,Edge->Cost)) return NO_MEM; 
		  }
  	  }  
  }
...
int AllSuccDist (PtDigraph pdig, unsigned int pv, double pdst, PtQueue *pqueue)
{
	...
	PtBiNode VerticeDado = OutPosition(pdig->Head,pv);
	if(VerticeDado == NULL) return NO_VERTEX;

	PtQueue Fila = QueueCreate(sizeof(unsigned int));
	if(Fila == NULL){ *pqueue = NULL; return NO_MEM;}

	PtBiNode Aresta = VerticeDado->PtAdj;
	PtEdge Edge;

	while(Aresta != NULL){
		Edge = Aresta->PtElem;
		if(Edge->Cost < pdst){
			if(QueueEnqueue(Fila,&Aresta->Number) != OK){
				QueueDestroy(&Fila);
				*pqueue = NULL;
				return NO_MEM;
			}
		}
		Aresta = Aresta->PtNext;
	}
...
int AllInEdgesVertex (PtDigraph pdig, unsigned int pv, PtQueue *pqueue)
{
	...
	for(Vertice = pdig->Head; Vertice!=NULL;Vertice = Vertice->PtNext){
		for(Aresta = Vertice->PtAdj;Aresta!=NULL;Aresta = Aresta->PtNext){
			if(Aresta->Number == pv){
				if(QueueEnqueue(Fila,&Vertice->Number) != OK || QueueEnqueue(Fila,&pv) != OK){
					QueueDestroy(&Fila);
					*pqueue = NULL;
					return NO_MEM;
				}
